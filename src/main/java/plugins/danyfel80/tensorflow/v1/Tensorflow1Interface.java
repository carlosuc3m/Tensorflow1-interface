package plugins.danyfel80.tensorflow.v1;
...
import org.bioimageanalysis.icy.tensorflow.v1.tensor.NDArrayBuilder;
 ...
 * @see NDArrayBuilder SequenceBuilder: Create sequences from tensors.
...
public class Tensorflow1Interface implements DeepLearningInterface
{
	...
	public List<Tensor> run(List<Tensor> inputTensors, List<Tensor> outputTensors) throws RunModelException {
        ...
        for (Tensor tt : inputTensors) {
        	...
        	org.tensorflow.Tensor<?> inT = TensorBuilder.build(tt.getDataAsNDArray());
        	inTensors.add(inT);
	...
	public static List<Tensor> fillOutputTensors(List<org.tensorflow.Tensor<?>> outputNDArrays, List<Tensor> outputTensors) throws RunModelException{
		...
		for (int i = 0; i < outputNDArrays.size(); i ++) {
			outputTensors.get(i).setNDArrayData(NDArrayBuilder.build(outputNDArrays.get(i), outputTensors.get(i).getManager().getManager()));
		}
		return outputTensors;
	}
